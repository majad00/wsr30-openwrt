...
/* Undefine the internal driver macros so they don't interfere with our real functions */
#ifdef delay_ms
#undef delay_ms
#endif
#ifdef delay_us
...
void delay_ms(u32 ms) { mdelay(ms); }
void delay_us(u32 us) { udelay(us); }

void _rtw_init_timer(struct timer_list *ptimer, void *padapter, void *pfunc, void *cntxt) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0))
    timer_setup(ptimer, (void (*)(struct timer_list *))pfunc, 0);
#else
    setup_timer(ptimer, (void (*)(unsigned long))pfunc, (unsigned long)cntxt);
#endif
}
/**
