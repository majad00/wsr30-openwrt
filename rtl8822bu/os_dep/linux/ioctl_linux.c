...
{
	/* parsing WPS IE */
	unsigned int cnt = 0, total_ielen;
	u8 *wpsie_ptr = NULL;
		...
		union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	u8 blnMatch = 0;
	u8 *p2pie;
	unsigned int p2pielen = 0, attr_contentlen = 0;
		...
		union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	u8 blnMatch = 0;
	u8 *p2pie;
	unsigned int p2pielen = 0, attr_contentlen = 2;
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	_queue				*queue	= &(pmlmepriv->scanned_queue);
	struct	wlan_network	*pnetwork = NULL;
	unsigned int					uintPeerChannel = 0;
#ifdef CONFIG_CONCURRENT_MODE
	struct roch_info		*prochinfo = &padapter->rochinfo;
#endif
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	u8						*p2pie;
	unsigned int						p2pielen = 0, attr_contentlen = 50;
	_irqL					irqL;
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	u8					attr_content[100] = { 0x00 };
	u8 *p2pie;
	unsigned int					p2pielen = 0, attr_contentlen = 100;
	_irqL				irqL;
#ifdef CONFIG_CONCURRENT_MODE
	struct roch_info 		*prochinfo = &padapter->rochinfo;
#endif
