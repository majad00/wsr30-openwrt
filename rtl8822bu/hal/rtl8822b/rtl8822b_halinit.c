 ...
 * more details.
 *port for WSR30 by majad at lut . fi
 *
 *****************************************************************************/
#define _RTL8822B_HALINIT_C_

#include "hal8822b_fw.h"	 // added by majad
#include <drv_types.h>		/* PADAPTER, basic_types.h and etc. */
#include <hal_data.h>		/* GET_HAL_SPEC(), HAL_DATA_TYPE */
#include "../hal_halmac.h"	/* HALMAC API */
#include "rtl8822b.h"

/* Manual Bridge: These replace the missing .h files */
struct hal_ops;
typedef struct _ADAPTER *PADAPTER;

/* From rtl8822b_phy.c */
extern void PHY_SwChnl8822B(PADAPTER pAdapter, unsigned char channel);
extern void PHY_SetBWMode8822B(PADAPTER pAdapter, u8 Bandwidth, u8 Offset);

/* From usb/rtl8822bu_halinit.c */
struct _io_ops;
extern void rtl8822bu_set_intf_ops(struct _io_ops *pops);
extern void rtl8822bu_enable_interrupt(PADAPTER padapter);
extern void rtl8822bu_disable_interrupt(PADAPTER padapter);
extern unsigned int rtl8822bu_inirp_init(PADAPTER padapter);
extern unsigned int rtl8822bu_inirp_deinit(PADAPTER padapter);
...
u8 rtl8822b_hal_init(PADAPTER adapter)
{
    ...
    extern u8 array_8822b_fw_nic[];
    extern u32 array_length_8822b_fw_nic;
    /* ---------------------------------- */
	struct dvobj_priv *d;
	PHAL_DATA_TYPE hal;
	int err;
...
//added by majad for safe refrencing of old Realtek 

void rtl8822b_set_chnl_bw(_adapter *padapter, u8 channel, enum channel_width Bandwidth, u8 Offset40, u8 Offset80)
{
    /* Call the channel change */
    PHY_SwChnl8822B(padapter, channel);
    /* Call the bandwidth change */
    PHY_SetBWMode8822B(padapter, Bandwidth, Offset40); 
}

void rtl8822b_set_hal_ops_halinit(struct hal_ops *ops)
{
    /* Core Section */
    ops->hal_init = &rtl8822b_hal_init;
    ops->hal_deinit = &rtl8822b_deinit;
    ops->hal_power_on = &rtl8822b_power_on;
    ops->hal_power_off = &rtl8822b_power_off;
    ops->init_default_value = &rtl8822b_init_default_value;

    /* Interface configuration with Type Cast */
    ops->intf_chip_configure = (void (*)(_adapter *))&rtl8822bu_set_intf_ops;

    /* USB Section */
    ops->inirp_init = &rtl8822bu_inirp_init;
    ops->inirp_deinit = &rtl8822bu_inirp_deinit;
    ops->enable_interrupt = &rtl8822bu_enable_interrupt;
    ops->disable_interrupt = &rtl8822bu_disable_interrupt;

    /* Hardware Register Handlers */
    ops->set_hw_reg_handler = &rtl8822b_sethwreg;
    ops->GetHwRegHandler = &rtl8822b_gethwreg;

    /* Default Variable Handlers */
    ops->get_hal_def_var_handler = &rtl8822b_gethaldefvar;
    ops->SetHalDefVarHandler = &rtl8822b_sethaldefvar;

    /* Combined Channel and Bandwidth Handler */
    ops->set_chnl_bw_handler = &rtl8822b_set_chnl_bw;
}
