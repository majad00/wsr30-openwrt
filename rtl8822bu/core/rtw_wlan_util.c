...
int check_ielen(u8 *start, unsigned int len)
{
	int left = len;
	u8 *pos = start;
	u8 id, elen;
...
void update_beacon_info(_adapter *padapter, u8 *pframe, unsigned int pkt_len, struct sta_info *psta)
{
	unsigned int i;
	unsigned int len;
...
#if CONFIG_DFS
void process_csa_ie(_adapter *padapter, u8 *ies, unsigned int ies_len)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);
		struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
...
unsigned char check_assoc_AP(u8 *pframe, unsigned int len)
{
	unsigned int	i;
	PNDIS_802_11_VARIABLE_IEs	pIE;
...
#ifdef CONFIG_80211AC_VHT
void get_vht_bf_cap(u8 *pframe, unsigned int len, struct vht_bf_cap *bf_cap)
{
	unsigned int i;
	PNDIS_802_11_VARIABLE_IEs pIE;
...
int rtw_ies_get_supported_rate(u8 *ies, unsigned int ies_len, u8 *rate_set, u8 *rate_num)
{
	u8 *ie, *p;
	unsigned int ie_len;
	int i, j;
...
void update_TSF(struct mlme_ext_priv *pmlmeext, u8 *pframe, unsigned int len)
{
	u8 *pIE;
	u32 *pbuf;
...
*/
inline u16 _rx_bcn_time_calculate(unsigned int bcn_len, u8 data_rate)
{
...
void rtw_rx_bcn_time_update(_adapter *adapter, unsigned int bcn_len, u8 data_rate)
{
	struct mlme_ext_priv *pmlmeext = &adapter->mlmeextpriv;
