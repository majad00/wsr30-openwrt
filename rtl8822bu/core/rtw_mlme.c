 ...
 * more details.
 * port for WSR30 made by majad @ lut.fi
 *****************************************************************************/
#define _RTW_MLME_C_
...
#define RTW_ROAM_DICONNECT_DELAY	20
	pmlmepriv->roam_flags = 0x01; /* Hardcoded fix for missing CONFIG_ROAMING_FLAG  added by majad*/
...
int rtw_set_key(_adapter *adapter, struct security_priv *psecuritypriv, int keyid, u8 set_tx, bool enqueue)
{
	u8	keylen;
	struct cmd_obj		*pcmd;
...
int rtw_restruct_wmm_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie, unsigned int in_len, unsigned int initial_out_len)
{
#ifdef CONFIG_WMMPS_STA
	struct mlme_priv		*pmlmepriv = &adapter->mlmepriv;
	struct qos_priv		*pqospriv = &pmlmepriv->qospriv;
...
int rtw_rsn_sync_pmkid(_adapter *adapter, u8 *ie, unsigned int ie_len, int i_ent)
{
...
int rtw_restruct_sec_ie(_adapter *adapter, u8 *out_ie)
{
	u8 authmode = 0x0;
	unsigned int	ielength = 0;
	int iEntry;
	...
	struct mlme_priv *pmlmepriv = &adapter->mlmepriv;
	struct security_priv *psecuritypriv = &adapter->securitypriv;
	unsigned int	ndisauthmode = psecuritypriv->ndisauthtype;
...
void rtw_build_wmm_ie_ht(_adapter *padapter, u8 *out_ie, unsigned int *pout_len)
{
...
unsigned int rtw_restructure_ht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie, unsigned int in_len, unsigned int *pout_len, u8 channel)
{
	u32 ielen, out_len;
...
void rtw_update_ht_cap(_adapter *padapter, u8 *pie, unsigned int ie_len, u8 channel)
{
	u8 *p, max_ampdu_sz;
	int len;
...
void rtw_append_exented_cap(_adapter *padapter, u8 *out_ie, unsigned int *pout_len)
{
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
#ifdef CONFIG_80211AC_VHT
