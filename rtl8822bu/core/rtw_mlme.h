...
struct wifidirect_info {
	...
	u8						uuid[16];					/* UUID */
	unsigned int						channel_list_attr_len;	/*	This field will contain the length of body of P2P Channel List attribute of group negotitation response frame. */
...
struct mlme_priv {
	...
	/* unsigned int wireless_mode; no used, remove it */

	u32	auto_scan_int_ms;

	_timer assoc_timer;

	unsigned int assoc_by_bssid;
	unsigned int assoc_by_rssi;
...
extern int rtw_set_key(_adapter *adapter, struct security_priv *psecuritypriv, int keyid, u8 set_tx, bool enqueue);
...
extern int rtw_restruct_wmm_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie, unsigned int in_len, unsigned int initial_out_len);
