...
void rtw_remove_bcn_ie(_adapter *padapter, WLAN_BSSID_EX *pnetwork, u8 index)
{
	...
	unsigned int offset, ielen, ie_offset, remainder_ielen = 0;
	u8	*pie = pnetwork->IEs;
...
#define KEEP_ALIVE_TRYCNT (3)
			...
			if ((psta->htpriv.ht_option == _TRUE) && (psta->htpriv.ampdu_enable == _TRUE)) {
				unsigned int priority = 1; /* test using BK */
				u8 issued = 0;
...
void rtw_process_public_act_bsscoex(_adapter *padapter, u8 *pframe, unsigned int frame_len)
{
	...
	unsigned int frame_body_len = frame_len - sizeof(struct rtw_ieee80211_hdr_3addr);
	u8 category, action;
