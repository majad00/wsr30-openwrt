...
#define __OSDEP_SERVICE_H_
    ...
    #define rtw_sprintf sprintf
    #define rtw_snprintf snprintf
    #define rtw_vsprintf vsprintf
#endif
...
#ifdef PLATFORM_WINDOWS
	if (((unsigned int)parg1) <= 0x7fffffff ||
	    ((unsigned int)parg2) <= 0x7fffffff ||
	    ((unsigned int)parg3) <= 0x7fffffff ||
	    ((unsigned int)parg4) <= 0x7fffffff) {
		ret = _FALSE;
...
/*
 * added by majad for wsr30 wifi
 */
#ifndef rtw_sprintf
    #define rtw_sprintf sprintf
#endif
 // no need to check error we are sure about the platform , added for wsr30
#include <linux/kernel.h>
#define rtw_sprintf sprintf
#define rtw_snprintf snprintf
#define rtw_vsprintf vsprintf
