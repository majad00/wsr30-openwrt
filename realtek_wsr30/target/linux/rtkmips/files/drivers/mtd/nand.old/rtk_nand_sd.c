...
// Add this line specifically!
	printk("\n\n!!! BOOTING WITH NEW 6MB PARTITION LAYOUT !!!\n\n");
...
{ name: "kernel",  offset: 0x05A00000, size: 0x00200000 }, /* dummy kernel with zeros */

{ name: "rootfs",  offset: 0x05E00000, size: 0x04200000 }, /* Physical Bank 2 rootfs2  */
...
void detect_nand_flash_map(struct mtd_info *mtd, struct mtd_partition *rtl819x_parts)
{
   ...
   if(bootbank == 1 )
	...
	rtk_init_flash_proc();
	/* COMMENT OUT THIS LOOP TO PREVENT AUTO-DETECTION */
	/*
...
#endif
	/* INSERT YOUR FORCE LOGIC HERE - RIGHT BEFORE THE FUNCTION ENDS */
	rtl819x_parts[2].size   = 0x600000;  /* Linux = 6MB */
	rtl819x_parts[3].offset = 0xE00000;  /* RootFS = 14MB offset */
	rtl819x_parts[3].size   = 0x2000000; /* RootFS Size */
...
#ifdef CONFIG_WRT_BARRIER_BREAKER
	/* 1. Comment out this line to stop the 32MB jump */
	// detect_nand_flash_map(rtk_mtd, rtl8196_parts1);

	/* 2. Add a printk so you can see it working in the log */
	printk("MTD_DEBUG: Registering manual 6MB/14MB layout\n");

	/* 3. This line registers your table 'rtl8196_parts1' */
