...
/* For Buffer Descriptor ring architecture */
...
#if defined(CONFIG_SDIO_HCI) || defined(CONFIG_GSPI_HCI)
	// removed by majad #define TXDESC_OFFSET TXDESC_SIZE
#endif
	...
	#endif
	// removed by majad #define TXDESC_OFFSET (TXDESC_SIZE + PACKET_OFFSET_SZ)
#endif
		...
		#ifndef TXDESC_OFFSET
		// removed by majad #define TXDESC_OFFSET (TX_WIFI_INFO_SIZE)
		#endif
	#else
		/* tx desc and payload are NOT in the same buf */
		// removed by majad #define TXDESC_OFFSET (0)
...
struct sta_xmit_priv {
	...
	/* unsigned int	sta_tx_bytes; */
	/* u64	sta_tx_pkts; */
	/* unsigned int	sta_tx_fail; */
...
struct	hw_txqueue	{
	...
	volatile int	 txsz[8];
	unsigned int	ff_hwaddr;
	unsigned int	cmd_hwaddr;
	int	ac_tag;
};
...
struct	xmit_priv	{
	...
	/* unsigned int mapping_addr; */
	/* unsigned int pkt_sz; */

	u8 *xframe_ext_alloc_addr;
	u8 *xframe_ext;
	unsigned int free_xframe_ext_cnt;
	...
	#ifndef __XMIT_TASKLET_DEFINED__
		_tasklet xmit_tasklet;
		#define __XMIT_TASKLET_DEFINED__
	#endif
...
extern void rtw_update_protection(_adapter *padapter, u8 *ie, unsigned int ie_len);
