...
static int rtw_cfg80211_set_probe_req_wpsp2pie(_adapter *padapter, char *buf, int len)
{
	...
	if (len > 0) {
		...
		if (p2p_ie) {
			...
			if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH, (u8 *)listen_ch_attr, (unsigned int *) &attr_contentlen)) {
...
#ifdef CONFIG_AP_MODE
void rtw_cfg80211_indicate_sta_assoc(_adapter *padapter, u8 *pmgmt_frame, unsigned int frame_len)
{
...
void rtw_cfg80211_rx_probe_request(_adapter *adapter, union recv_frame *rframe)
{
	...
	unsigned int frame_len = rframe->u.hdr.len;
	s32 freq;
