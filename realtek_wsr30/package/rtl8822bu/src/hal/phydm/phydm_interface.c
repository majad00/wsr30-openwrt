 ...
 *Port for WSR30 is done by Majad Qureshi at lut . fi
 *
 *****************************************************************************/
...
{
#if (DM_ODM_SUPPORT_TYPE & (ODM_AP))
	return phy_query_rf_reg(dm->priv, e_rf_path, reg_addr, bit_mask);
#elif (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	void *adapter = dm->adapter;
...
void odm_initialize_timer(struct dm_struct *dm, struct phydm_timer_list *timer,
			  void *func, void *context, const char *name)
{
	/* Use the generic pointer to call the driver's init function */
	struct odm_dm_struct *p_dm = (struct odm_dm_struct *)dm;
	_rtw_init_timer(timer, p_dm->priv, func, context);
}
...
void odm_fill_h2c_cmd(struct dm_struct *dm, u8 element_id, u32 cmd_len, u8 *cmd_buf)
{
	struct odm_dm_struct *p_dm = (struct odm_dm_struct *)dm;

	if (p_dm && p_dm->priv) {
		rtw_hal_fill_h2c_cmd(p_dm->priv, element_id, cmd_len, cmd_buf);
	}
}

/* Ensure there is a blank line here at the end of the file */
