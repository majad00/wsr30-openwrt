...
unsigned int rtw_hal_iface_init(_adapter *adapter)
{
...
unsigned int rtw_hal_init(_adapter *padapter)
{
	unsigned int status = _SUCCESS;
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
...
unsigned int rtw_hal_deinit(_adapter *padapter)
{
	unsigned int	status = _SUCCESS;
...
{
		#if 0 /* MANUALLY DISABLED TO FIX BUILD ERROR ON MIPS */
	struct macid_ctl_t *macid_ctl = adapter_to_macidctl(adapter);
...
// edits to disbale MIPS build error
