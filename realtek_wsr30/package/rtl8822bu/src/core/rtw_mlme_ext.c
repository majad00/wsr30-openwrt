...
static u8 cmp_pkt_chnl_diff(_adapter *padapter, u8 *pframe, unsigned int packet_len)
{
...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	u8 *pframe = precv_frame->u.hdr.rx_data;
	unsigned int len = precv_frame->u.hdr.len;
	u8 is_valid_p2p_probereq = _FALSE;

#ifdef CONFIG_ATMEL_RC_PATCH
	u8 *target_ie = NULL, *wps_ie = NULL;
	u8 *start;
	unsigned int search_len = 0, wps_ielen = 0, target_ielen = 0;
...
unsigned int OnAuthClient(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	u8 *pframe = precv_frame->u.hdr.rx_data;
	unsigned int pkt_len = precv_frame->u.hdr.len;
...
unsigned int on_action_spct_ch_switch(_adapter *padapter, struct sta_info *psta, u8 *ies, unsigned int ies_len)
{
...
unsigned int on_action_spct(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	struct sta_priv *pstapriv = &padapter->stapriv;
	u8 *pframe = precv_frame->u.hdr.rx_data;
	unsigned int frame_len = precv_frame->u.hdr.len;
...
void issue_p2p_GO_response(_adapter *padapter, u8 *raddr, u8 *frame_body, unsigned int len, u8 result)
{
	struct p2p_channels *ch_list = &(adapter_to_rfctl(padapter)->channel_list);
	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	u8			action = P2P_PUB_ACTION_ACTION;
...
#define TOKEN_REL 0x01
	...
	switch (tbtx_action_code)
	{
		case TOKEN_REQ:
			// parse duration
			tx_duration = le32_to_cpu(*(unsigned int *)(pframe + WLAN_HDR_A3_LEN + 2));
	...
	*(fctrl) = 0;
	...
	if (MLME_IS_AP(padapter) || MLME_IS_MESH(padapter)) {
		...
		if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) && wpsie && wpsielen > 0) {
			unsigned int wps_offset, remainder_ielen;
			u8 *premainder_ie, *pframe_wscie;

			wps_offset = (unsigned int)(wpsie - cur_network->IEs);
	...
	*(fctrl) = 0;
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
		...
		if ((pmlmepriv->wps_probe_resp_ie != NULL) && pwps_ie && (wps_ielen > 0)) {
			...
			wps_offset = (unsigned int)(pwps_ie - cur_network->IEs);
			...
			wps_ielen = (unsigned int)pmlmepriv->wps_probe_resp_ie[1];/* to get ie data len */
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	if ((val16 & 0x03) == cap_ESS) {
		...
		int mesh_id_ie_len, mesh_conf_ie_len;
