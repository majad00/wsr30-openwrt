...
#define DBG_RFRAME_DEL_WFD_IE 0
	...
	unsigned int ies_len_ori = rframe->u.hdr.len - (ies - rframe->u.hdr.rx_data);
...
int recv_decache(union recv_frame *precv_frame)
{
	struct sta_info *psta = precv_frame->u.hdr.psta;
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	_adapter *adapter = psta->padapter;
	int tid = pattrib->priority;
...
int rtw_tdls_rx_data_validate_hdr(
	_adapter *adapter,
...
static int validate_mgmt_protect(_adapter *adapter, union recv_frame *precv_frame)
{
...
int wlanhdr_to_ethhdr(union recv_frame *precvframe, enum rtw_rx_llc_hdl llc_hdl)
{
...
static int MPwlanhdr_to_ethhdr(union recv_frame *precvframe)
{
	int	rmv_len;
	u16 len;
