...
unsigned int rtw_get_cckrate_size(u8 *rate, u32 rate_length)
{
	int i = 0;
	while(i < rate_length){
...
(
	...
	unsigned int len,
	const u8 *source,
	unsigned int *frlen /* frame length */
)
{
	*pbuf = (u8)index;
 ...
 */
u8 *rtw_get_ie_ex(const u8 *in_ie, unsigned int in_len, u8 eid, const u8 *oui, u8 oui_len, u8 *ie, unsigned int *ielen)
{
	unsigned int cnt;
 ...
 */
int rtw_ies_remove_ie(u8 *ies, unsigned int *ies_len, unsigned int offset, u8 eid, u8 *oui, u8 oui_len)
{
	int ret = _FAIL;
	u8 *target_ie;
	u32 target_ielen;
	...
	while (1) {
		...
		if (target_ie && target_ielen) {
			...
			unsigned int remain_len = search_len - (remain_ies - start);
...
void rtw_filter_suppport_rateie(WLAN_BSSID_EX *pbss_network, u8 keep)
{
	...
	unsigned int iscck, isofdm, ie_orilen = 0, remain_len;
	u8 *remain_ies;
...
u8 rtw_update_rate_bymode(WLAN_BSSID_EX *pbss_network, u32 mode)
{
	...
	int ie_len;
	unsigned int network_ielen = pbss_network->IELength;
...
int rtw_rsne_info_parse(const u8 *ie, unsigned int ie_len, struct rsne_info *info)
{
	const u8 *pos = ie;
	u16 ver;
	u16 cnt;
...
int rtw_get_sec_ie(u8 *in_ie, unsigned int in_len, u8 *rsn_ie, u16 *rsn_len, u8 *wpa_ie, u16 *wpa_len)
{
	u8 authmode, sec_idx;
...
u8 *rtw_get_wps_ie_from_scan_queue(u8 *in_ie, unsigned int in_len, u8 *wps_ie, unsigned int *wps_ielen, enum bss_type frame_type)
{
	u8	*wps = NULL;
 ...
 */
u8 *rtw_get_wps_ie(const u8 *in_ie, unsigned int in_len, u8 *wps_ie, unsigned int *wps_ielen)
{
	unsigned int cnt;
 ...
 */
u8 *rtw_get_wps_attr(u8 *wps_ie, unsigned int wps_ielen, u16 target_attr_id , u8 *buf_attr, u32 *len_attr)
{
	u8 *attr_ptr = NULL;
 ...
 */
u8 *rtw_get_wps_attr_content(u8 *wps_ie, unsigned int wps_ielen, u16 target_attr_id , u8 *buf_content, unsigned int *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
 ...
 */
u8 *rtw_get_owe_ie(const u8 *in_ie, unsigned int in_len, u8 *owe_ie, unsigned int *owe_ielen)
{
	unsigned int cnt;
...
static int rtw_ieee802_11_parse_vendor_specific(u8 *pos, unsigned int elen,
 ...
 * Returns: the address of the specific WPS attribute found, or NULL
 */
u8 *rtw_get_p2p_attr(u8 *p2p_ie, unsigned int p2p_ielen, u8 target_attr_id , u8 *buf_attr, u32 *len_attr)
{
	u8 *attr_ptr = NULL;
	u8 *target_attr_ptr = NULL;
 ...
 */
u8 *rtw_get_p2p_attr_content(u8 *p2p_ie, unsigned int p2p_ielen, u8 target_attr_id , u8 *buf_content, unsigned int *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
...
#define DBG_DEL_P2P_IE 0
	...
	u8 *target_ie;
	u32 target_ie_len;
	unsigned int ies_len = ies_len_ori;
	int index = 0;
	...
	while (1) {
		...
		if (target_ie && target_ie_len) {
			...
			unsigned int remain_len = ies_len - (next_ie - ies);
...
#define DBG_DEL_P2P_ATTR 0
	...
	while (1) {
		...
		if (target_attr && target_attr_len) {
			...
			unsigned int remain_len = ielen - (next_attr - ie);
...
inline u8 *rtw_bss_ex_get_p2p_ie(WLAN_BSSID_EX *bss_ex, u8 *p2p_ie, unsigned int *p2p_ielen)
{
...
#define DBG_BSS_EX_DEL_P2P_IE 0
	...
	unsigned int ies_len_ori = BSS_EX_TLV_IES_LEN(bss_ex);
...
#define DBG_BSS_EX_DEL_P2P_ATTR 0
	...
	u8 *ies = BSS_EX_TLV_IES(bss_ex);
	unsigned int ies_len = BSS_EX_TLV_IES_LEN(bss_ex);

	u8 *ie;
	unsigned int ie_len, ie_len_ori;
	...
	while (1) {
		...
		if (ie) {
			...
			unsigned int remain_len = bss_ex->IELength - (next_ie_ori - bss_ex->IEs);
 ...
 * Returns: the address of the specific WPS attribute found, or NULL
 */
u8 *rtw_get_wfd_attr(u8 *wfd_ie, unsigned int wfd_ielen, u8 target_attr_id, u8 *buf_attr, u32 *len_attr)
{
	u8 *attr_ptr = NULL;
	u8 *target_attr_ptr = NULL;
 ...
 */
u8 *rtw_get_wfd_attr_content(u8 *wfd_ie, unsigned int wfd_ielen, u8 target_attr_id, u8 *buf_content, unsigned int *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
...
inline u8 *rtw_bss_ex_get_wfd_ie(WLAN_BSSID_EX *bss_ex, u8 *wfd_ie, unsigned int *wfd_ielen)
{
...
u8 *rtw_set_multi_ap_ie_ext(u8 *pbuf, unsigned int *frlen, u8 val)
{
	u8 cont_len = 7;
