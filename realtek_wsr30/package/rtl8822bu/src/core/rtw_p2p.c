...
u32 process_probe_req_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
	u8 *p;
	u32 ret = _FALSE;
	u8 *p2pie;
...
u32 process_assoc_req_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len, struct sta_info *psta)
{
	...
	while (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CAPABILITY, (u8 *)&cap_attr, (unsigned int *) &attr_contentlen)) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO, NULL, (unsigned int *)&attr_contentlen)) {
			...
			if (pattr_content) {
				...
				rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO , pattr_content, (unsigned int *)&attr_contentlen);
...
u32 process_p2p_devdisc_req(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
	u8 *frame_body;
...
u32 process_p2p_devdisc_resp(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
	return _TRUE;
}

u8 process_p2p_provdisc_req(struct wifidirect_info *pwdinfo,  u8 *pframe, unsigned int len)
{
	u8 *frame_body;
	u8 *wpsie;
	unsigned int	wps_ielen = 0, attr_contentlen = 0;
...
u8 process_p2p_group_negotation_req(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
	_adapter *padapter = pwdinfo->padapter;
	...
	unsigned int	wps_devicepassword_id_len = 0;
#ifdef CONFIG_WFD
#ifdef CONFIG_TDLS
	...
	while (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH, (u8 *)listen_ch_attr, (unsigned int *) &attr_contentlen) && attr_contentlen == 5)
...
u8 process_p2p_group_negotation_resp(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
	_adapter *padapter = pwdinfo->padapter;
	u8	result = P2P_STATUS_SUCCESS;
...
u8 process_p2p_group_negotation_confirm(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
#ifdef CONFIG_CONCURRENT_MODE
	_adapter *padapter = pwdinfo->padapter;
...
u8 process_p2p_presence_req(struct wifidirect_info *pwdinfo, u8 *pframe, unsigned int len)
{
	u8 *frame_body;
	u8 dialogToken = 0;
...
static void rtw_change_p2pie_op_ch(_adapter *padapter, const u8 *frame_body, u32 len, u8 ch)
{
	...
	while (p2p_ie) {
		...
		attr_contentlen = 0;
		pattr = NULL;
		pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_OPERATING_CH, NULL, (unsigned int *)&attr_contentlen);
		if (pattr != NULL)
			*(pattr + 4) = ch;
...
static void rtw_change_p2pie_ch_list(_adapter *padapter, const u8 *frame_body, u32 len, u8 ch)
{
	...
	while (p2p_ie) {
		...
		pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST, NULL, (unsigned int *)&attr_contentlen);
		if (pattr != NULL) {
			int i;
			u32 num_of_ch;
...
#define DBG_XFRAME_DEL_WFD_IE 0
	...
	u8 *ies = NULL;
	unsigned int ies_len_ori = 0;
	unsigned int ies_len = 0;
...
u8 *dump_p2p_attr_ch_list(u8 *p2p_ie, unsigned int p2p_ielen, u8 *buf, u32 buf_len)
{
	unsigned int attr_contentlen = 0;
	u8 *pattr = NULL;
	int w_sz = 0;
	u8 ch_cnt = 0;
	u8 ch_list[40];
