...
static void construct_mic_iv(
	...
	u8 *pn_vector,
	unsigned int frtype);/* add for CONFIG_IEEE80211W, none 11w also can use */
...
static int aes_cipher(u8 *key, unsigned int	hdrlen,
		       u8 *pframe, unsigned int plen)
{
	/*	static unsigned char	message[MAX_MSG_SIZE]; */
	unsigned int	qc_exists, a4_exists, i, j, payload_remainder,
	...
	unsigned int	frsubtype  = get_frame_sub_type(pframe);
...
#if (NEW_CRYPTO == 0)
static int aes_decipher(u8 *key, unsigned int	hdrlen,
			 u8 *pframe, unsigned int plen)
{
...
u32 rtw_bip_verify(enum security_type gmcs, u16 pkt_len,
	u8 *whdr_pos, int flen, const u8 *key, u16 keyid, u64 *ipn)
{
	u8 * BIP_AAD,*mme;
	u32 res = _FAIL;
	unsigned int len, ori_len;
	u16 pkt_keyid = 0;
	u64 pkt_ipn = 0;
